...
import { Card, CardBody, CardFooter, CardHeader, Chip, cn, Divider, Tab, Tabs } from "@heroui/react";
          ...
          >
            ...
            <CardBody>
              <div className="flex flex-row flex-wrap gap-2 text-small">
                <Chip
                  color={selectedPeserta[activePesertaIndex].penilaian_anda === "Lulus" ? "success" : selectedPeserta[activePesertaIndex].penilaian_anda === "Tidak Lulus" ? "danger" : "default"}
                  startContent={
                    selectedPeserta[activePesertaIndex].penilaian_anda === "Lulus" ? (
                      <CheckCircle size={18} />
                    ) : selectedPeserta[activePesertaIndex].penilaian_anda === "Tidak Lulus" ? (
                      <CircleX size={18} />
                    ) : null
                  }
                  variant="flat"
                >
                  Penilaian Anda: {selectedPeserta[activePesertaIndex].penilaian_anda ?? 'Belum Anda Nilai'}
                </Chip>
                { selectedPeserta[activePesertaIndex].penilaian_anda === "Lulus" && selectedPeserta[activePesertaIndex].rekomendasi_anda === true &&
                  <Chip color="success" startContent={<CheckCircle size={18}/>} variant="flat">Anda Merekomendasikan</Chip>
                }
              </div>
            </CardBody>
            <Divider />
            <CardFooter>
              <div className="flex flex-row flex-wrap gap-2 text-small">
                <Chip color="primary" variant="flat">
                  Jumlah Meluluskan: {selectedPeserta[activePesertaIndex].count_akademik_lulus}
                </Chip>
                <Chip color="primary" variant="flat">
                  Jumlah Tidak Meluluskan: {selectedPeserta[activePesertaIndex].count_akademik_tidak_lulus}
                </Chip>
                <Chip
                  color={selectedPeserta[activePesertaIndex].hasil_sistem === "Lulus" ? "success" : selectedPeserta[activePesertaIndex].hasil_sistem === "Tidak Lulus Akademik" ? "danger" : "default"}
                  startContent={
              ...
              >
                <CardBody className="overflow-hidden">
                  <div className="flex flex-col gap-4">
                    {
                      selectedPeserta[activePesertaIndex].akademik.map((akademik) => <RiwayatAkademikKertosonoCard key={akademik.id} akademik={akademik}/>)
                    }
                    {
                      selectedPeserta[activePesertaIndex].akademik.length == 0 && <EmptyState/>
                    }  
                  </div>
                </CardBody>
              </Card>
