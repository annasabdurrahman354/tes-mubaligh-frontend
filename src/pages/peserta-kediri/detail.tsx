        ...
        <div className="flex flex-col">
          ...
          >
            ...
            <CardBody>
              <div className="flex flex-row flex-wrap gap-2 text-small">
                <Chip color="primary" variant="flat">
                  Makna: {Number(selectedPeserta[activePesertaIndex].avg_nilai_makna).toFixed(2) ?? 0}
                </Chip>
                <Chip color="primary" variant="flat">
                  Keterangan: {Number(selectedPeserta[activePesertaIndex].avg_nilai_keterangan).toFixed(2) ?? 0}
                </Chip>
                <Chip color="primary" variant="flat">
                  Penjelasan: {Number(selectedPeserta[activePesertaIndex].avg_nilai_penjelasan).toFixed(2) ?? 0}
                </Chip>
                <Chip color="primary" variant="flat">
                  Pemahaman: {Number(selectedPeserta[activePesertaIndex].avg_nilai_pemahaman).toFixed(2) ?? 0}
                </Chip>
              </div>
            </CardBody>
            <Divider />
            <CardFooter>
              <div className="flex flex-row flex-wrap gap-2 text-small">
                <Chip color="primary" variant="flat">
                  Nilai Akhir: {Number(selectedPeserta[activePesertaIndex].avg_nilai).toFixed(2) ?? 0}
                </Chip>
                <Chip
                  color={selectedPeserta[activePesertaIndex].hasil_sistem === "Lulus" ? "success" : selectedPeserta[activePesertaIndex].hasil_sistem === "Tidak Lulus Akademik" ? "danger" : selectedPeserta[activePesertaIndex].hasil_sistem === "Perlu Musyawarah" ? "warning" : "default"}
                  startContent={
                    selectedPeserta[activePesertaIndex].hasil_sistem === "Lulus" ? (
                      <CheckCircle size={18} />
                    ) : selectedPeserta[activePesertaIndex].hasil_sistem === "Tidak Lulus" ? (
                      <CircleX size={18} />
                    ) : selectedPeserta[activePesertaIndex].hasil_sistem === "Perlu Musyawarah" ? (
            ...
            <Tab key="akhlak" title="Akhlak">
              ...
              >
                <CardBody className="overflow-hidden">
                  <div className="flex flex-col gap-4">
                    {
                      selectedPeserta[activePesertaIndex].akhlak.map((akhlak) => <RiwayatAkhlakKediriCard key={akhlak.id} akhlak={akhlak}/>)
                    }
                    {
